...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  ...
  const post = await getPostBySlug('de', slug);
  if (!post) return {};

  const canonical = post.canonical ?? `/de/${post.slug}/`;
  const alternates = await buildAlternatesForSlugPath({ locale: 'de', slugPath: post.slug, canonical });
...
export default async function PostPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  return (
    <>
      ...
      <ArticleLayout post={post} locale="de" />
    </>
  );
}
