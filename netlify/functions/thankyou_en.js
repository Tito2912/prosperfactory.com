...
<html lang="${LANG}">
  <head>
    ...
    <title>Thank you — Prosper Factory</title>
  ...
  <body>
    <main>
      ...
      <p>Your download is ready. If it doesn’t start automatically, use the button below.</p>
      <p><a class="btn" href="${downloadUrl}" rel="nofollow">Download the PDF</a></p>
      <p class="fine">Tip: keep your Stripe confirmation link (it contains the session token).</p>
      <noscript><p class="fine">JavaScript is disabled. The button above still works.</p></noscript>
      <script>setTimeout(function(){window.location.replace(${JSON.stringify(
        downloadUrl,
      )})}, 1200);</script>
      <p class="fine"><a href="${HOME_FALLBACK}" rel="nofollow">Back to homepage</a></p>
    </main>
  </body>
</html>`;
