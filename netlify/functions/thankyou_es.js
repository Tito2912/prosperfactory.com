const PRODUCT_ID = "prod_TGT50OMoqWV3bx";
const LANG = "es";
const HOME_FALLBACK = "/es/";
const CHECKOUT_FALLBACK = "/es/pago";
...
<html lang="${LANG}">
  <head>
    ...
    <title>Acceso protegido — Prosper Factory</title>
  ...
  <body>
    <main>
      ...
      <p>Esta página está disponible después del pago. Ábrela desde el enlace de confirmación de Stripe (contiene el token de sesión).</p>
      <div class="actions">
        <a class="btn" href="${CHECKOUT_FALLBACK}" rel="nofollow">Ir al pago</a>
        <a class="ghost" href="${mailto}" rel="nofollow">Contactar soporte</a>
      </div>
      <p class="fine"><a href="${HOME_FALLBACK}" rel="nofollow" style="color:var(--accent);text-decoration:none">Volver al inicio</a></p>
    </main>
  </body>
</html>`;
...
<html lang="${LANG}">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Gracias — Prosper Factory</title>
    <meta name="robots" content="noindex, nofollow" />
    <style>
      :root{color-scheme:dark;--bg:#070707;--panel:#111;--accent:#d4af37;--text:#f5f5f5;--muted:#b9b9b9}
      body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:var(--bg);color:var(--text);display:flex;min-height:100vh;align-items:center;justify-content:center;padding:24px}
  ...
  <body>
    <main>
      <h1>Gracias — pago confirmado</h1>
      <p>Tu descarga está lista. Si no comienza automáticamente, utiliza el botón de abajo.</p>
      <p><a class="btn" href="${downloadUrl}" rel="nofollow">Descargar el PDF</a></p>
      <p class="fine">Consejo: guarda el enlace de confirmación de Stripe (contiene el token de sesión).</p>
      <noscript><p class="fine">JavaScript está desactivado. El botón funciona igualmente.</p></noscript>
      <script>setTimeout(function(){window.location.replace(${JSON.stringify(
        downloadUrl,
      )})}, 1200);</script>
      <p class="fine"><a href="${HOME_FALLBACK}" rel="nofollow">Volver al inicio</a></p>
    </main>
  </body>
</html>`;
