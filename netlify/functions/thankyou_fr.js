const PRODUCT_ID = "prod_TGSjL24r4PjuHq";
const LANG = "fr";
const HOME_FALLBACK = "/fr/";
const CHECKOUT_FALLBACK = "/fr/paiement";
...
<html lang="${LANG}">
  <head>
    ...
    <title>Accès protégé — Prosper Factory</title>
  ...
  <body>
    <main>
      <h1>Accès protégé</h1>
      <p>Cette page est disponible après paiement. Ouvre-la depuis le lien de confirmation Stripe (il contient le jeton de session).</p>
      <div class="actions">
        <a class="btn" href="${CHECKOUT_FALLBACK}" rel="nofollow">Ouvrir le paiement</a>
        <a class="ghost" href="${mailto}" rel="nofollow">Contacter le support</a>
      </div>
      <p class="fine"><a href="${HOME_FALLBACK}" rel="nofollow" style="color:var(--accent);text-decoration:none">Retour à l’accueil</a></p>
    </main>
  </body>
</html>`;
...
<html lang="${LANG}">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Merci — Prosper Factory</title>
    <meta name="robots" content="noindex, nofollow" />
    <style>
      :root{color-scheme:dark;--bg:#070707;--panel:#111;--accent:#d4af37;--text:#f5f5f5;--muted:#b9b9b9}
      body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:var(--bg);color:var(--text);display:flex;min-height:100vh;align-items:center;justify-content:center;padding:24px}
  ...
  <body>
    <main>
      ...
      <p>Votre téléchargement est prêt. Si rien ne se lance automatiquement, utilisez le bouton ci-dessous.</p>
      <p><a class="btn" href="${downloadUrl}" rel="nofollow">Télécharger le PDF</a></p>
      <p class="fine">Astuce : gardez le lien de confirmation Stripe (il contient le jeton de session).</p>
      <noscript><p class="fine">JavaScript est désactivé. Le bouton ci-dessus fonctionne quand même.</p></noscript>
      <script>setTimeout(function(){window.location.replace(${JSON.stringify(
        downloadUrl,
      )})}, 1200);</script>
      <p class="fine"><a href="${HOME_FALLBACK}" rel="nofollow">Retour à l’accueil</a></p>
    </main>
  </body>
</html>`;
